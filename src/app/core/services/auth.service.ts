...
registerInstructor(instructorData: Instructor): Observable<any> {
  return this.http.post(`${this.baseUrl}/auth/instructor/signup`, instructorData)
    .pipe(
      switchMap((response: any) => {

        // After successful registration, send OTP verification code
        return this.sendVerificationCode(instructorData.registrationDetails.email);
      }),
...
registerSeeker(studentData: Student): Observable<any> {
  return this.http.post(`${this.baseUrl}/auth/student/signup`, studentData)
    .pipe(
      switchMap((response: any) => {
        // After successful registration, send OTP verification code
        return this.sendVerificationCode(studentData.registrationDetails.email);
      }),
      catchError(error => this.handleError(error))
    );
}
